...
Vision-based detection methods, particularly single-stage architectures such as YOLO~\cite{redmon2016}, demonstrate strong performance on automotive benchmarks~\cite{geiger2012} but experience degradation under extreme illumination variations.
...
While sensor fusion has been proposed to address individual modality limitations~\cite{huang2024a, liang2022}, most fusion research focuses on automotive applications using terrestrial datasets at a 10:1 ratio for maritime applications.
...
The widespread adoption of convolutional neural networks (CNNs) for visual object detection followed Krizhevsky et al.'s breakthrough with ImageNet, which demonstrated that deep CNNs (DCNN) trained on large-scale datasets could achieve remarkable detection performance~\cite{krizhevsky2017}.
...
Girshick et al.~\cite{girshick2014} introduced R-CNN, which applies CNNs to region proposals; however, the computational overhead limited throughput.
...
Subsequent YOLO iterations introduced architectural refinements, including multi-scale detection heads and anchor-free prediction mechanisms.

YOLOv8, released in 2023 as a commercial implementation by Ultralytics, introduced architectural innovations addressing limitations of earlier single-stage detectors.
...
These architectural properties make YOLOv8 suitable for real-time maritime perception, where computational efficiency is crucial. However, small-object detection constraints motivate the investigation of resolution scaling strategies and complementary sensing modalities.

Transfer learning from large-scale terrestrial datasets (ImageNet, COCO~\cite{lin2015}) has become standard practice for maritime detection, yet introduces domain adaptation challenges.
...
% High Dynamic Range (HDR) imaging aims to capture a broader range of luminance levels than Standard Dynamic Range (SDR), thereby preserving scene details in both bright and dark regions. 
% In vision-based object detection, HDR is theoretically advantageous for handling challenging lighting conditions such as glare, strong shadows, or backlit scenarios—common in maritime environments. 
% However, in practice, studies have reported mixed results: networks trained on SDR imagery sometimes outperform those trained on HDR data, potentially because conventional networks are typically pre-trained on large SDR datasets~\cite{landaeta, liebergall}. 
% This mismatch may introduce biases, limiting the benefits of HDR processing for object detection tasks.
% The scarcity of HDR training data limits the effectiveness of transfer learning.

Visual detection in maritime environments presents challenges that are fundamentally different from those in terrestrial settings. Background motion from waves and wakes, low contrast objects, and environmental degradation are major sources of error according to Prasad et al.\cite{prasad2017}. Atmospheric conditions also complicate detection. For example, Zhang et al.\cite{zhang2022a} addressed maritime fog by using image preprocessing and attention mechanisms. Detecting small objects remains a challenge. As noted by Rekavandi et al.\cite{rekavandi2022}, objects that occupy less than ten percent of the image area rarely provide enough information for accurate classification. This problem is even greater in maritime scenarios because vessels and navigation markers often appear near the horizon and take up only a few pixels. Shao et al.\cite{shao2022} approached these issues with multi-scale detection techniques and deformable convolution modules. Progress in maritime detection has been limited because there are no standardized large-scale datasets like KITTI or COCO\cite{zhang2021, yang2024, su2023}.
High Dynamic Range imaging is another approach used to address the lighting conditions in maritime environments. HDR captures both bright and dark details in the same image. This can help detect objects in situations with glare, strong shadows, or backlighting, which are common at sea. In practice, however, networks trained only on Standard Dynamic Range images sometimes perform better than those trained on HDR images. This may be because most existing training datasets use SDR images\cite{landaeta, liebergall}. The lack of HDR training data and the bias toward SDR limit the effectiveness of HDR for object detection and also hinder transfer learning.
...
Model performance on small maritime objects degrades substantially at reduced input resolutions~\cite{shao2022, rekavandi2022}.
The optimal resolution-accuracy-latency trade-off for embedded maritime platforms remains unquantified.
Research evaluating the use of high dynamic range cameras to address dynamic lighting conditions in maritime environments is limited, and published results are conflicting.~\cite{landaeta, liebergall}.
...
% These gaps motivate a systematic evaluation of YOLO variants under maritime-specific conditions, with a focus on the feasibility of embedded platforms.
...
BEVFusion~\cite{liang2022, liu2023b} achieves state-of-the-art accuracy but requires approximately 300 watts of power consumption and achieves only 15-20 frames per second on desktop-class GPUs~\cite{liu2023a}—this power budget alone exceeds total available power for many small autonomous vessels.
...
Liu et al.~\cite{liu2023a} note that BEVFusion achieves state-of-the-art performance but requires approximately 300 watts of power consumption, exceeding embedded platform capabilities by an order of magnitude.
For battery-powered maritime platforms where a typical total system power budget ranges from 30 to 50 watts, mid-fusion's computational demands preclude direct deployment without substantial architectural optimization.
...
Automotive perception benefits from large-scale benchmarks (KITTI~\cite{geiger2012}, Waymo~\cite{sun2020}, nuScenes~\cite{caesar2020}) providing millions of labeled examples with synchronized camera-LiDAR data.
...
Early and mid-fusion methods often exceed the computational budgets of embedded platforms, requiring desktop-class GPUs that consume 200-400 watts for real-time operation—specifications incompatible with small, battery-powered autonomous vessels.
