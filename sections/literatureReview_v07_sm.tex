...
Maritime visual detection faces distinct challenges: background motion from waves, low contrast objects, and environmental degradation~\cite{prasad2017}.
...
Atmospheric conditions and small object detection at distance further compound difficulty~\cite{zhang2022a, shao2022, rekavandi2022}.
...
BEVFusion~\cite{liang2022, liu2023b} achieves state-of-the-art performance but requires approximately 300 watts power consumption~\cite{liu2023a}—precluding direct deployment on battery-powered maritime platforms where 30-50 watts represents typical total system power budgets.
...
Extrinsic calibration establishes geometric relationships through rotation matrix and translation vector, traditionally using fiducial targets (checkerboards, AprilTags)~\cite{iyer2018}.
...
Automotive research demonstrates late fusion offers superior robustness under sensor dropout~\cite{huang2024a, wang2020a}, yet these findings derive from terrestrial datasets where environmental characteristics differ substantially from maritime scenarios.
Whether late fusion maintains robustness advantages under maritime-specific challenges—water reflections, HDR extremes, platform motion—remains empirically unvalidated.
Early and mid-fusion methods often require desktop-class GPUs consuming 200-400 watts, incompatible with small battery-powered vessels~\cite{liu2023a}, yet systematic evaluation of accuracy-latency-power trade-offs for late fusion on representative embedded hardware (NVIDIA Jetson AGX Xavier, 30-watt power consumption) is lacking.
...
Automotive research established general calibration tolerances~\cite{cui2022}, but maritime platform dynamics may impose different requirements; wave-induced motion creates time-varying calibration errors requiring empirical characterization.
...
The approach is motivated by late fusion's advantages for maritime deployment: modular architecture isolating sensor failures, computational efficiency compatible with embedded hardware constraints, and robustness to calibration and synchronization errors compared to tightly coupled fusion approaches.
