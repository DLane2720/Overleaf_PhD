...
Furthermore, despite theoretical advantages of HDR imaging in high-contrast maritime environments, systematic evaluation of HDR preprocessing for maritime YOLO variants is absent from reviewed literature.
Available studies report contradictory findings regarding HDR benefits~\cite{landaeta, liebergall}, likely attributable to pre-training dataset biases where networks trained on SDR data fail to effectively extract features from HDR inputs~\cite{krizhevsky2017}.
...
Additionally, limited evidence exists regarding fine-tuning data requirements for adapting terrestrial pre-trained models to maritime environments~\cite{guo2023}.
The relationship between training set size, environmental diversity, and generalization performance requires systematic investigation to establish practical guidelines for maritime dataset collection and model development.
...
Automotive algorithms exploit abundant asphalt and concrete returns to establish spatial reference frames and segment foreground objects from background, but maritime applications lack this dense reference, requiring alternative spatial reasoning strategies where objects appear isolated.
Systematic evaluation of detection performance versus water surface characteristics—including return rate variations with wave conditions, sun angle, and water turbidity—is absent from reviewed literature.
...
Furthermore, GB-CACHE~\cite{coyleE} demonstrates real-time feasibility and cross-domain applicability for deterministic geometric methods, yet maritime-specific validation under varying environmental conditions is limited.
Comparative evaluation against learned approaches for maritime object classes would clarify accuracy-simplicity-reliability trade-offs and inform algorithm selection for different operational scenarios.
...
While automotive fusion research often assumes desktop-class GPU availability, maritime platforms operate under strict power budgets where every watt of perception processing directly reduces mission endurance or payload capacity.
Furthermore, while Huang et al.~\cite{huang2024a} demonstrate that late fusion tolerates synchronization error better than early fusion, systematic evaluation sweeping timing offsets to establish acceptable tolerances for maritime platforms is absent.
...
Wave-induced motion creates time-varying calibration errors that differ from the static calibration errors typical of ground vehicles; acceptable calibration accuracy and repeatability requirements for operational deployment need empirical determination.
...
Quantifying when and where fusion provides measurable benefits versus added complexity is essential for informed system design decisions, particularly when evaluating whether fusion improvements justify additional sensors, processing requirements, calibration maintenance, and system complexity.
