...
In parallel with vision-based detection, this research examines GB-CACHE as a real-time object detection and classification algorithm
% optimized for sparse maritime environments.
Its name is derived from the methodology it utilizes: grid-based clustering and concave hull extraction.
This approach was designed for marine environments, where LiDAR returns are sparse and concentrated on only a few nearby objects~\cite{coyleE}.
...
The algorithm begins by first filtering unstructured point cloud data from the Livox units to remove points corresponding to waves and reflections from the water's surface.
The remaining points are then projected into a globally fixed frame, where they are added to a two-dimensional occupancy grid with a resolution of \textcolor{blue}{$5 cm$}.
Each added point is associated with its nearest occupancy grid cell using a primary set of $x$ and $y$ coordinates, and a tertiary $z$ coordinate to preserve vertical geometry.

% The algorithm begins by projecting unstructured point cloud data into a globally fixed frame and occupancy grid with a resolution of \textcolor{blue}{$5 cm$}. %, upon which an occua spatially organized grid defined in the global fixed frame.
% Each point is associated with the occupancy grid using a primary set of $x$ and $y$ coordinates, and a tertiary $z$ coordinate to preserve vertical geometry.
...
Each segmented object is assigned a unique identifying number and is associated with various object-related data.
First is a perimeter-matched polygon formed from the $x$ and $y$ vertices of the concave hull of the object's two-dimensional boundary.
The intensity of each LiDAR return associated with the object, as well as the maximum $z$ height observed.
Features are extracted from this metadata and used to identify the class of the object that was observed using a multivariate Gaussian classifier (MGC).
Finally, the object ID, class, and associated metadata are processed together and published as a ROS topic \texttt{obj\_w\_class} for mapping and navigational purposes.
...
\begin{figure}
    ...
    \caption{Block diagram of GB-CACHE process flow. Clock colors correspond to measured segments of the GB-CACHE process: Ground Plane Removal (blue), Add Points (lime green), and clustering/Convex Hull Extraction/Classification (teal).}
...
% Analysis
The algorithmic design of \ac{GB-CACHE} prioritizes efficiency to enable real-time processing on autonomous surface vessels.
Grid-based spatial indexing reduces clustering complexity and promotes memory-efficient data storage and retrieval operations.
...
To determine the efficiency of the method, the execution time of specific subroutines of the algorithm was timed and examined.
These subroutines, "Filter Points", "Acquire Points", and "Update Map", are indicated in Figure~\ref{fig:gbcache_flow_analysis} with the functional blocks they measure.

The LiDAR data is transmitted at a rate of 100 Hz, and the output ROS topic \texttt{Obj\_w\_class} that this algorithm generates is published at 5 Hz, we can say something about how fast each of the three subroutines must operate at.
By comparing their operational speed to the rate of published data from this method, we can make claims about the model's efficiency.

The data recorded in each rosbag was played back on the same Minion PC hardware that recorded it, perfectly simulating the data topics required to execute the ROS nodes responsible for performing the object detection.
Each rosbag within the dataset was replayed on the same Minion hardware that recorded it, providing only the raw LiDAR data and navigational topics to the GB-CACHE ROS node.
The execution time of each subroutine was measured, along with the number of points input to each function.
Overall, more than 1.7 million executions of "Filter Points" and "Add Points"  and more than 45 thousand executions of "Update Map" were measured. 

The first two subroutines examined, "Filter points" and "Add Points", execute sequentially. 
With access to the source code, we can determine that these processes execute sequentially and without parallelization.
This means they must process raw data from three Livox Horizon units at a rate of 100 Hz; therefore, these subroutines must be executed at least once every 3.33 milliseconds.  
The "Update Map" subroutine is updated with data three times every 0.01 seconds, but only outputs data at a rate of five Hz, and has an allotted execution time of 200 milliseconds.

The average execution time for each of the three subroutines, their allotted time, and the percentage of that time used per cycle are presented in Table~\ref{tbl:gbcache_subroutine_results}.
...
The Filter Points and Add Points subroutines together require an average of 0.149 ms per cycle, which corresponds to approximately 4.5\% of the 3.33 ms processing window allocated for each LiDAR frame. 
This indicates that online point filtering and insertion impose a minimal computational load relative to the cycle time. 
...
Both stages, therefore, maintain substantial timing margins, confirming that the mapping pipeline can operate in real time without approaching its latency constraints.
Histograms of the execution times for each subroutine are provided in Figures~\ref{fig:gbcache_filter_points, fig:gbcache_add_points} and \ref{fig:gbcache_update_map} along with a scatterplot of execution time plotted against the number of points processed.

Plotting each subroutine against the number of points processed shows how this algorithm computationally scales.
...
The "Update Map" operation, while more computationally intensive, exhibits strong linear correlation with object geometric complexity, specifically polygon perimeter and surface area.
...
GB-CACHE employs an \acl{MGC} to specify what class of object it has identified. 
...
The magnitude of an object's observed individual features may change depending on its distance from the observer or the viewing angle, but their relative values still provide valuable information.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \section{GB-CACHE Performance} \label{performance_gbcache}
% \subsubsection{}\
% \
