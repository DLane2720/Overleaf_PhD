

\begin{acronym}[GB-CACHE]
\acro{GB-CACHE}{grid-based clustering and concave hull extraction} 
\acro{AGS}{autonomous ground system}
% \acro{ASV}{autonomous surface vessel}
\acro{USV}{uncrewed surface vessel}
\acro{ERAU}{Embry-Riddle Aeronautical University}
\acro{GPS}{Global Positioning System}
\acro{WAM-V}{wave-adaptive modular vessel}
\acro{SDR}{standard dynamic range}
\acro{HDR}{high dynamic range}
\acro{CMOS}{complementary metal-oxide-semiconductor}
\acro{DCG}{dual-conversion gain}
\acro{DOL-HDR}{digital-overlap HDR}
\acro{IMU}{inertial measurement unit}
\acro{INS}{inertial navigation system}
\acro{IoU}{intersection over union}
\acro{CIoU}{intersection over union}
\acro{SOD}{small object detection}
\acro{LiDAR}{light detection and ranging}
\acro{pps}{points per second}
\acro{mAP}{mean average precision}
\acro{MPC}{model predictive control}
\acro{RGB}{red, green, blue}
\acro{SSIM}{structural similarity index measure}
\acro{ROI}{region of interest}
\acro{ROS}{Robot Operating System}
\acro{YOLO}{You Only Look Once}
\acro{YOLOv8}{You Only Look Once ver. 8.0}
\acro{CNN}{convolutional neural network}
\acro{R-CNN}{region-based convolutional neural network}
\acro{LWIR}{long-wave infrared}
\acro{fps}{frames per second}
\acro{EFL}{effective focal length}
\acro{CSV}{comma separated value}
\acro{FOV}{field of view}
\acro{LAN}{local area network}
\acro{WAN}{wide area network}
\acro{SEI}{supplemental enhancement information}
\acro{NAL}{network abstraction layer}
\acro{GbE}{gigabit Ethernet}
\acro{Mbps}{megabits per second}
\acro{MBps}{megabytes per second}
\acro{Gbps}{gigabits per second}
\acro{GBps}{gigabytes per second}
\acro{NTP}{Network Time Protocol}
\acro{PTP}{Precision Time Protocol}
\acro{RTP}{Real-time Transport Protocol}
\acro{RTSP}{Real-time Streaming Protocol}
\acro{UDP}{User Datagram Protocol}
\acro{RMS}{root mean square}
\acro{ICP}{iterative closest point}
\acro{FLU}{forward-left-up}
\acro{FRD}{forward-right-down}
\acro{NED}{northing-easting-down}
\acro{MGC}{multivariate gaussian classifier}
\acro{PDF}{probability distribution function}
\acro{BEV}{bird's-eye-view}
\acro{CLOC}{Camera-LiDAR Object Candidate}
\acro{MI}{mutual information}
\acro{PMI}{pairwise mutual information}
\acro{CPU}{central processing unit}
\acro{GPU}{graphics processing unit}
\acro{RAM}{random access memory}
\acro{vRAM}{video random access memory}
\acro{NVMe}{non-volatile memory express}
\acro{mRMR}{Minimum Redundancy Maximum Relevance}
\acro{COLREGs}{International Regulations
for Preventing Collisions at Sea}
\acro{IMO}{International Maritime Organization}
\acro{MASS}{Maritime Autonomous Surface Ships}
\end{acronym}