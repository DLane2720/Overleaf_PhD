...
This enables the fused detection label to reflect the confidence assigned to each model, such that the influence of each system is proportional to its estimated reliability.
However, for this approach to work, both detections must share a common reference frame. 
For this reason, detections from GB-CACHE are projected into the image frame. 



Having established the independent capabilities and limitations of both camera-based and LiDAR-based detection systems in maritime environments, this section outlines the implementation of a late-fusion method that synthesizes their outputs at the decision level.
This late fusion approach is a decision-level integration strategy that combines object detection outputs from both YOLO and GB-CACHE after each system has independently produced a detection and classification.
The objective is to leverage the demonstrated individual performance of each system and improve the detection through the application of a Kalman filter, while preserving the computational efficiency demonstrated by each modality.

The fusion pipeline begins with temporal synchronization of asynchronous detection messages generated by the independent sensing modalities. 
Each detection message carries timestamp metadata that enables temporal alignment between YOLOv8 and GBCACHE outputs. 
Only detection pairs occurring within a defined temporal window proceed to spatial association evaluation, ensuring that the fused prediction integrates observations of the same scene state rather than temporally disparate measurements.
Spatially, candidate detection pairs undergo geometric correspondence assessment through computation of the Euclidean distance between bounding box centroids. This metric provides a computationally efficient initial filter for identifying detections that potentially correspond to the same physical object. Detection pairs satisfying the spatial proximity threshold advance to intersection over union calculation, which quantifies the degree of overlap between the two bounding boxes. Pairs exceeding a predefined IoU threshold are deemed sufficiently correspondent to warrant fusion, while those failing this criterion are discarded or processed as independent single-modality detections. This rule-based association framework permits graceful degradation in scenarios where one sensing modality fails or when temporal or geometric correspondence cannot be established, ensuring continuous detection capability while leveraging multi-modal information when conditions permit.


the error associated with each detection source is quantified on labeled data.
When GB-CACHE and YOLO methods both detect an ROI and agree on its classification, a third ROI is generated.
The measured performance of each model on the ground-truth data is used to weight that model's influence on the new bounding box.
The final fused estimate then applies a weighted average in the image frame, where each weight reflects the empirical reliability of that source.
For each frame, detections from vision and LiDAR are paired before fusion. 
% Pairs are formed when two conditions hold: the predicted classes match and the intersection-over-union (IoU) is at least 0.70.
Pairs are formed only if two conditions are met: the predicted classes must match, and the intersection-over-union (IoU) between bounding boxes must be at least 0.70. When several candidates meet these criteria, the pair with the highest IoU is selected. If multiple pairs have identical IoU values, the one with the smallest center-to-center distance is chosen.
For this analysis, only paired detections are evaluated, as we want to ensure detections within a 60-meter range. As YOLO does not have the ability for this spatial condition, single-mode returns are ignored.
...
By assuming that the distributions of errors $\mathbf{E}_S$ will trend towards a normally distributed Gaussian ($N(\mu,\sigma^2)$) for a sufficiently large set of $N$ samples, the error is characterized as  
\begin{align}
\mathbf{\mu}_{U} &= \frac{1}{N_{U}} \sum_{i=1}^{N_{U}} E_{U}^{(i)} \\
\mathbf{\sigma}^2_{U} &= \frac{1}{N_{U} - 1} \sum_{i=1}^{N_{U}}
\bigl(E_{U}^{(i)} - \mu_{U}\bigr)^2
...
This method of statistical error analysis and application guarantees that the resulting fused ROI will be improved over the GB-CACHE method alone.
Provided that ground truth data is abundant and at least equivalent in quality to the data provided for the transfer learning of the YOLO model, it guarantees that the fused result is better than either method, in isolation.

To characterize the error of the detection edges generated by the GB-CACHE method, additional training data were selected and labeled.
\textcolor{red}{A summary of the data used for this evaluation is provided in Table \#. Initial results from the application of this method indicate an improvement ...}
