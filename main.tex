...
\paragraph{Sensor Layout Philosophy}
...
\paragraph{Field of View Configuration}
...
\paragraph{Sensor Overlap Optimization}
...
\paragraph{Spatial Reference Frame}
...
\begin{enumerate}
    \item \textbf{Fusion-optimized coverage}: Maximum spatial overlap between vision and \ac{LiDAR} modalities enables direct comparison of detection performance and validates late fusion approaches.

    \item \textbf{Forward-facing emphasis}: Maritime collision avoidance and navigation planning require detailed perception of the forward corridor, making 360-degree coverage unnecessary for this research application.

    \item \textbf{Density over breadth}: Concentrating \ac{LiDAR} sampling within the camera field of view provides higher effective point cloud density compared to omnidirectional sensors with equivalent point return rates.

    \item \textbf{Temporal aggregation support}: The Livox scan pattern and \ac{GPS}/\ac{INS} integration enable temporal accumulation of observations during vessel motion, further increasing effective spatial resolution.
\end{enumerate}
