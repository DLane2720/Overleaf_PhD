...
The rise of autonomous systems is transforming numerous industries, and the maritime sector is no exception. \Acp{ASV} have the potential to improve safety and operational efficiency by performing the dull, dirty, and dangerous tasks traditionally done by humans.
...
Although many propose late fusion to enhance detection robustness, \textcolor{red}{(cite)} few studies consistently or comprehensively characterize the computational cost of fusion in real-time \ac{ASV} deployments \textcolor{red}{(cite)}.
Additionally, reports on spatial calibration and temporal synchronization\textcolor{red}{(cite)}, which are required by such fusion techniques, vary widely in scope and detail, undermining reproducibility and complicating cross-study comparisons. 
...
on Minion hardware (NVIDIA GeForce GTX 1080 GPU), establishing baseline performance and selecting the small variant for its equivalent accuracy to medium with improved inference speed, as detailed in Chapter~\ref{realtime_object_detection}.
...
For this research, a synchronized \ac{LiDAR}-camera dataset was collected aboard the \acl{WAMV} platform during operational deployments, providing 778 annotated images with 1934 labeled maritime objects, as described in Section~\ref{sec:sensor_data_dataset}.
...
\caption{ERAU's \ac{WAMV} Research Vessel Minion as configured for data collection at the 2024 Maritime RobotX Challenge.}
\label{fig:minion}
\end{figure}
...
% Time synchronization is similarly structured and discussed in more detail in Section~\ref{time_sync}.
Time synchronization is similarly structured,
with a master clock distributed over the network to subsystems and sensors to ensure that
data acquired across sensor modalities remain temporally aligned.
...
Camera–LiDAR calibration begins by positioning a calibration target the planar checkerboard target within both the LiDAR field of view and the camera image.  
...
This approach produces a rough estimate of the necessary transform. Manual refinement is required to achieve results demonstrated in Figures \textcolor{red}{insert image here.}%
...
This implementation replaces the caption-based approach with direct timestamp embedding using Supplemental Enhancement Information (SEI) Network Abstraction Layer (NAL) units defined in the H.264/H.265 standard.
...
% In this configuration, the GPS clock serves as the master time source, which is distributed over the network via an \ac{NTP} service.
% This synchronizes the separate system clocks. 
% A \ac{PTP} service is used to achieve higher timing precision on the network-attached Livox Horizon units.
...
In this configuration, the Jetson acts as the \ac{PTP} master clock server, broadcasting timing packets over the camera-enclosure network to all three Livox devices, which function as \ac{PTP} clients. \ac{PTP} uses hardware-timestamped Ethernet packets to measure network delay symmetrically and correct for it in real time, achieving sub-microsecond synchronization between LiDAR units. The Livox Viewer utility confirms synchronization state during system startup, and visual indicators within the GUI report “active 1588 signal” once synchronization is established .
...
An initial evaluation of the video stream and time-stamping system was conducted under conditions simulating expected network load.
Video data was recorded for 14.5 seconds at 10 \ac{fps} (twice the anticipated operational rate) for a total of 8,720 frames. 
The NVIDIA computer in the camera enclosure was accessed via SSH terminal, which displayed a log of each timestamp recorded and sent over the RTSP video stream. 
Similarly, the Atlas PC was used to display the timestamp being received over the RTSP video stream.
...
This allows any internal clock drift to be easily identified by comparing the transmitted timestamp to the time displayed in the video.
...
The results of this test showed a total delay in video frame reception of 127 milliseconds, and logs between the computers showed no dropped frames and a 100\% match in timestamps.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% To initialize alignment for each bag file, the first five ROS image messages within each bag were manually paired to the corresponding frames in the source video. This established the time offset between the ROS message timestamps and video frame timestamps, computed as the average difference across the initialization frames:
...
% This offset was then used to programmatically pair the remaining image messages in the bag with their video frame counterparts. For each ROS message ($i > N_{init}$), the predicted video timestamp was calculated as:

% $$\hat{t}_{video,i} = t_{ROS,i} + \Delta t$$

% A search window $W_i$ of candidate video frames was defined around this predicted timestamp. The corresponding video frame $f_i$ was then selected by maximizing the Structural Similarity Index (SSIM):
