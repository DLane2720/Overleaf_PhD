...
Finally, the resulting spatial calibration accuracy and temporal alignment obtained are discussed in section \ref{}.
...
% % Therefore, a set of points in reference frame $A$ ($\mathbf{P}_{A}$) can be expressed in reference frame $B$ ($\mathbf{P}_{B}$) through 
% % \begin{equation}
% %     \mathbf{P}_{B} =
% %     \mathbf{R}_{A}^{B}\mathbf{P}_{A} + \mathbf{t}_{A}^{B}.
% % \end{equation}
...
% the LiDAR and camera reference frames, consisting of a rotation matrix $\mathbf{R}_{C}^{L}$ and translation vector $\mathbf{t}_{C}^{L}$ that together form the homogeneous transformation $\mathbf{T}_{C}^{L}$.  
...
where $\mathbf{R}_{B}^{A} \in \mathbb{R}^{3\times3}$ is the rotation matrix describing the orientation of frame $A$ relative to frame $B$, and $\mathbf{t}_{B}^{A} \in \mathbb{R}^{3}$ is the translation vector locating the origin of frame $A$ with respect to frame $B$.  
...
A point $\mathbf{P}_{A}$ expressed in frame $A$ can therefore be transformed into frame $B$ as

\begin{equation}
    \mathbf{P}_{B} =
    \mathbf{R}_{B}^{A}\mathbf{P}_{A} + \mathbf{t}_{B}^{A}.
\end{equation}
...
This initial alignment provides a coarse estimate of $\mathbf{T}_{C}^{L}$ sufficient for projecting LiDAR points into the image plane with moderate accuracy.
...
I employed the YOLOv8 architecture from Ultralytics for vision-based maritime object detection due to its demonstrated balance between detection accuracy and computational efficiency in real-time applications.
...
This single-stage approach enables real-time performance by eliminating the computational overhead of region proposal generation and iterative refinement stages required by two-stage detectors such as R-CNN variants.
...
For the Minion platform, I configured the model for inference at the native resolution of the Leopard Imaging \ac{IMX490} \ac{HDR} camera (2880×1860 pixels).
...
Frame extraction occurs at 1 \ac{Hz} to provide temporal diversity while avoiding excessive correlation between consecutive frames that would artificially inflate apparent dataset size without corresponding information gain.
...
Classification of detected maritime objects employs a \ac{MvGC} framework that models the joint probability distribution of extracted features for each object class.
...
Feature selection for the \ac{MvGC} focuses on geometric descriptors and intensity statistics that exhibit discriminative power across maritime object categories.
Geometric features including object height, width, depth, volume, and boundary complexity capture shape characteristics that differentiate buoys from vessels and distinguish marker types.
...
The selection of ten features balances classification accuracy against computational overhead and the risk of overfitting with limited training data.
...
To obtain the two-dimensional pixel coordinates of the distorted image ($u, v$) in the camera frame $C$ from a three-dimensional coordinate ($X_L, Y_L, Z_L$) in the LiDAR frame $L$:

% 3D LiDAR point to distorted image pixel
% \paragraph{Projection from LiDAR to image.}
Let $\mathbf{P}_L = [X_L, Y_L, Z_L, 1]^T$, and $\mathbf{P}_C$ are these points rotated and translated about $\mathbf{T}_{C}^{L}$, such that  
$\mathbf{P}_C = \mathbf{T}_{C}^{L}\mathbf{P}_L$:

:
\begin{equation*}
[X_C, Y_C, Z_C, 1]^T \;= \; \mathbf{T}_{C}^{L} \;[X_L, Y_L, Z_L, 1]^T
% ,\qquad
% \mathbf{T}_{C}^{L} =
% \begin{bmatrix}
